...
if SERVER then
    ...
    if npc_wise_mystical_tree_force_download:GetBool() then
        resource.AddWorkshop(workshopID)
    end

    util.AddNetworkString("wise_mystical_tree_nag")
    util.AddNetworkString("wise_mystical_tree_navgen")
    ...
    local function isPointNearSpawn(point, distance)
        ...
        for _, spawnPoint in pairs(GAMEMODE.SpawnPoints) do
            ...
            if point:DistToSqr(spawnPoint:GetPos()) <= distanceSqr then return true end
        end
    ...
    local function isPositionExposed(pos)
        for _, ply in pairs(player.GetAll()) do
            if IsValid(ply) and ply:Alive() and ply:IsLineOfSightClear(pos) then return true end -- This spot can be seen!
        end

        return false
    end

    local VECTOR_wise_mystical_tree_HEIGHT = Vector(0, 0, 96)

    local function isPointSuitableForHiding(point)
        trace.start = point
        trace.endpos = point + VECTOR_wise_mystical_tree_HEIGHT
    ...
    local function buildHidingSpotCache()
        ...
        print(string.format("npc_wise_mystical_tree: found %d suitable (%d unsuitable) hiding \z
		places in %d areas over %.2fms!", goodSpots, badSpots, #areas, (SysTime() - rStart) * 1000))
    end
    ...
    local function isValidTarget(ent)
        ...
        -- Ignore dead NPCs, other wise mystical trees, and dummy NPCs.
        local class = ent:GetClass()

        return ent:IsNPC() and ent:Health() > 0 and class ~= "npc_wise_mystical_tree" and not class:find("bullseye")
    end

    hook.Add("PlayerSpawnedNPC", "wise_mystical_treeMissingNavmeshNag", function(ply, ent)
        if not IsValid(ent) then return end
        if ent:GetClass() ~= "npc_wise_mystical_tree" then return end
        if navmesh.GetNavAreaCount() > 0 then return end
        -- Try to explain why wise mystical tree isn't working.
        net.Start("wise_mystical_tree_nag")
        net.Send(ply)
    end)
    ...
    local function navEndGenerate()
        ...
        if not navmesh.IsGenerating() then
            print("npc_wise_mystical_tree: Navmesh generation completed in " .. timeElapsedStr)
        else
            print("npc_wise_mystical_tree: Navmesh generation aborted after " .. timeElapsedStr)
        end
    ...
    local function navGenerate()
        ...
        if next(seeds, nil) == nil then
            print("npc_wise_mystical_tree: Couldn't find any places to seed nav_generate")
        ...
        for seed in pairs(seeds) do
            ...
            if not tr.StartSolid and tr.Hit then
                print(string.format("npc_wise_mystical_tree: Adding seed %s at %s", seed, pos))
                navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
            else
                print(string.format("npc_wise_mystical_tree: Couldn't add seed %s at %s", seed, pos))
            end
        end

        -- The least we can do is ensure they don't have to listen to this noise.
        for _, wise_mystical_tree in pairs(ents.FindByClass("npc_wise_mystical_tree")) do
            wise_mystical_tree:Remove()
        end
        ...
        if navmesh.IsGenerating() then
            ...
            hook.Add("ShutDown", "wise_mystical_treeNavGen", navEndGenerate)
        else
            print("npc_wise_mystical_tree: nav_generate failed to initialize")
    ...
    concommand.Add("npc_wise_mystical_tree_learn", function(ply, cmd, args)
        if navmesh.IsGenerating() then return end
        -- Rcon or single-player only.
        local isConsole = ply:EntIndex() == 0

        if game.SinglePlayer() then
            print("npc_wise_mystical_tree: Beginning nav_generate requested by " .. ply:Name())
            -- Disable expensive computations in single-player. wise mystical tree doesn't use
        ...
        elseif isConsole then
            print("npc_wise_mystical_tree: Beginning nav_generate requested by server console")
        else
            return
        end
        ...
        net.Start("wise_mystical_tree_navgen")
    ...
    function ENT:Initialize()
        ...
        -- Were a little timid on drops... Give the player a chance. :)
    ...
    function ENT:GetNearestTarget()
        ...
        local maxAcquireDist = npc_wise_mystical_tree_acquire_distance:GetInt()
        ...
        for _, ent in pairs(acquirableEntities) do
            ...
            -- if `npc_wise_mystical_tree_spawn_protect' = 1.
            --TODO: Only for the first few seconds?
            if npc_wise_mystical_tree_spawn_protect:GetBool() and ent:IsPlayer() and isPointNearSpawn(ent:GetPos(), 200) then continue end
    ...
    function ENT:AttackNearbyTargets(radius)
        local attackForce = npc_wise_mystical_tree_attack_force:GetInt()
        ...
        for _, ent in pairs(nearEntities) do
            if isValidTarget(ent) then
                ...
                -- Give the player a good whack. wise mystical tree means business.
            ...
            elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
                if not npc_wise_mystical_tree_smash_props:GetBool() then continue end
    ...
    function ENT:IsHidingSpotFull(hidingSpot)
        ...
        if not IsValid(occupant) or occupant == self then return false end
    ...
    function ENT:GetNearestUsableHidingSpot()
        ...
        for _, hidingSpot in pairs(g_hidingSpots) do
            ...
            if hidingSpot.nearSpawn or isHidingSpotFull(self, hidingSpot) then continue end
            --TODO: Disallow hiding places near spawn?
            local hidingSpotDistSqr = distToSqr(hidingSpot.pos, myPos)

            if hidingSpotDistSqr < nearestHidingDistSqr and not isPositionExposed(hidingSpot.pos) then
    ...
    function ENT:AttemptJumpAtTarget()
        ...
        local maxAttackDistance = npc_wise_mystical_tree_attack_distance:GetInt()
        ...
        if xyDistSqr <= math.pow(maxAttackDistance + 200, 2) and zDifference >= maxAttackDistance then
            ...
            self.loco:Jump()
            self.loco:SetJumpHeight(300)
            self:EmitSound(jumpHeight > HIGH_JUMP_HEIGHT and self.JumpHighSound or self.JumpSound, 350, 100)
        end
    end
    ...
    function ENT:RecomputeTargetPath()
        if CurTime() - self.LastPathingInfraction < PATH_INFRACTION_TIMEOUT then return end -- No calculations for you today.
        local targetPos = self.CurrentTarget:GetPos()
    ...
    --TODO: Split this up more. Eww.
    function ENT:BehaveUpdate()
        if ai_disabled:GetBool() then return end -- We may be a bot, but we're still an "NPC" at heart.
        local currentTime = CurTime()
        local scanInterval = npc_wise_mystical_tree_expensive_scan_interval:GetFloat()
        ...
        if IsValid(self.CurrentTarget) then
            ...
            local attackInterval = npc_wise_mystical_tree_attack_interval:GetFloat()

            if currentTime - self.LastAttack > attackInterval then
                local attackDistance = npc_wise_mystical_tree_attack_distance:GetInt()
            ...
            local repathInterval = npc_wise_mystical_tree_chase_repath_interval:GetFloat()
            ...
            if self:IsOnGround() and npc_wise_mystical_tree_allow_jump:GetBool() and currentTime - self.LastJumpScan >= scanInterval then
                self:AttemptJumpAtTarget()
                self.LastJumpScan = currentTime
            end
        else
            local hidingScanInterval = npc_wise_mystical_tree_hiding_scan_interval:GetFloat()
            ...
            if self.HidingSpot ~= nil then
                local hidingInterval = npc_wise_mystical_tree_hiding_repath_interval:GetFloat()
    ...
    function ENT:OnStuck()
        ...
        local newCursor = self.MovePath:GetCursorPosition() + 40 * math.pow(2, self.StuckTries)
    ...
    local function DevPrint(devLevel, msg)
        if developer:GetInt() >= devLevel then
            print("npc_wise_mystical_tree: " .. msg)
        end
    end
    ...
    local function updatePanicMusic()
        if #ents.FindByClass("npc_wise_mystical_tree") == 0 then
            -- Whoops. No need to run for now.
            DevPrint(4, "Halting music timer.")
            timer.Remove("wise_mystical_treePanicMusicUpdate")
        ...
        local userVolume = math.Clamp(npc_wise_mystical_tree_music_volume:GetFloat(), 0, 1)
        ...
        for _, ent in pairs(nearEntities) do
            if IsValid(ent) and ent:GetClass() == "npc_wise_mystical_tree" then
        ...
        local musicVolume = math.min(1, totalDistanceScore / MUSIC_wise_mystical_tree_MAX_DISTANCE_SCORE)
        ...
        if musicVolume > 0 then
            ...
            if not LocalPlayer():Alive() then
                -- Quiet down so we can hear wise mystical tree taunt us.
    ...
    local function startTimer()
        if not timer.Exists("wise_mystical_treePanicMusicUpdate") then
            timer.Create("wise_mystical_treePanicMusicUpdate", 0.05, REPEAT_FOREVER, updatePanicMusic)
    ...
    function ENT:DrawTranslucent()
        render.SetMaterial(MAT_wise_mystical_tree)
        -- Get the normal vector from wise mystical tree to the player's eyes, and then compute
        ...
        xyNormal:Normalize()
        -- wise mystical tree should only look 1/3 of the way up to the player so that they
    ...
    surface.CreateFont("wise_mystical_treeHUD", {
        font = "Arial",
        size = 56
    })

    surface.CreateFont("wise_mystical_treeHUDSmall", {
        font = "Arial",
        size = 24
    })
    ...
    local flavourTexts = {
        {"Gotta learn fast!", "Learning this'll be a piece of cake!", "This is too easy."},
        {"This must be a big map.", "This map is a bit bigger than I thought.",},
        {"Just how big is this place?", "This place is pretty big."},
        {"This place is enormous!", "A guy could get lost around here."},
        {"Surely I'm almost done...", "There can't be too much more...", "This isn't gm_bigcity, is it?", "Is it over yet?", "You never told me the map was this big!"}
    }
    ...
    local function navGenerateHUDOverlay()
        draw.SimpleTextOutlined("wise_mystical_tree is studying this map.", "wise_mystical_treeHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
        draw.SimpleTextOutlined("Please wait...", "wise_mystical_treeHUD", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 2, color_black)
        local elapsed = SysTime() - generateStart
        local elapsedStr = string_ToHMS(elapsed)
        draw.SimpleTextOutlined("Time Elapsed:", "wise_mystical_treeHUDSmall", ScrW() / 2, ScrH() * 3 / 4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
        draw.SimpleTextOutlined(elapsedStr, "wise_mystical_treeHUDSmall", ScrW() / 2, ScrH() * 3 / 4, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 1, color_black)
        -- It's taking a while.
        ...
        draw.SimpleTextOutlined(flavourText, "wise_mystical_treeHUDSmall", ScrW() / 2, ScrH() * 4 / 5, color_yellow, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
    end

    net.Receive("wise_mystical_tree_navgen", function()
        local startSuccess = net.ReadBool()

        if startSuccess then
            generateStart = SysTime()
            lastBracket = 0
            hook.Add("HUDPaint", "wise_mystical_treeNavGenOverlay", navGenerateHUDOverlay)
        else
            Derma_Message("Oh no. Wise mystical tree doesn't even know where to start with \z
		this map.\n\z
    ...
    local function requestNavGenerate()
        RunConsoleCommand("npc_wise_mystical_tree_learn")
    end
    ...
    local function navWarning()
        Derma_Query("It will take a while (possibly hours) for wise mystical tree to figure \z
		this map out.\n\z
		...
		Anything that has been built will be deleted.", "Warning!", "Go ahead!", requestNavGenerate, "Not right now.", nil)
    end

    net.Receive("wise_mystical_tree_nag", function()
        if not nagMe then return end

        if game.SinglePlayer() then
            Derma_Query("Uh oh! Wise mystical tree doesn't know this map.\n\z
			Would you like him to learn it?", "This map is not yet wise mystical tree-compatible!", "Yes", navWarning, "No", nil, "No. Don't ask again.", stopNagging)
        else
            Derma_Query("Uh oh! wise mystical tree doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to wise mystical tree.\n\z
			\n\z
			If you ARE the server host, you can run npc_wise_mystical_tree_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.", "This map is currently not wise mystical tree-compatible!", "Ok", nil, "Ok. Don't say this again.", stopNagging)
        end
    end)
end
